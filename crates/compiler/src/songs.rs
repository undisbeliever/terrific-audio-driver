...
pub fn compile_song(
    ...
    let inst_map = build_data_instruments_map(instruments);

    let mml = match parse_mml(mml_text, &inst_map, &pitch_table) {
        Ok(mml) => mml,
